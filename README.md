...
<https://vcvrack.com/downloads> (under "Rack SDK") and unzip it somewhere.
It must match your Rack 2 version. `RACK_DIR` in the commands below is the
path to that unzipped folder.

### 2. Install a toolchain

- **Linux** — `gcc`/`g++`, `make`, and `git`
  (e.g. `sudo apt install build-essential git`).
- **macOS** — install the Xcode Command Line Tools
  (`xcode-select --install`) and [Homebrew](https://brew.sh), then
  `brew install git wget cmake autoconf automake libtool jq`. Building a
  universal (Intel + Apple Silicon) plugin works out of the box with the
  matching SDK.
- **Windows** — install [MSYS2](https://www.msys2.org), open the
  **MSYS2 MINGW64** shell, and run
  `pacman -S git make tar mingw-w64-x86_64-gcc mingw-w64-x86_64-jq`.
  Run all the build commands from that MINGW64 shell (not cmd or
  PowerShell).

This repo vendors the only third-party DSP it needs (Émilie Gillet's
MIT-licensed Plaits and Clouds code under `eurorack/`), so there are no
extra dependencies to fetch.

### 3. Build and install

From the repo folder, with `RACK_DIR` pointing at your unzipped SDK:

```bash
# Linux
make -j$(nproc) RACK_DIR=/path/to/Rack-SDK
make install RACK_DIR=/path/to/Rack-SDK
```

```bash
# macOS
make -j$(sysctl -n hw.ncpu) RACK_DIR=/path/to/Rack-SDK
make install RACK_DIR=/path/to/Rack-SDK
```

```bash
# Windows (MSYS2 MINGW64 shell)
make -j$(nproc) RACK_DIR=/c/path/to/Rack-SDK
make install RACK_DIR=/c/path/to/Rack-SDK
```

`make install` copies the built plugin into Rack's user plugin folder for
your OS:

| OS      | User plugin folder                                  |
|---------|-----------------------------------------------------|
| Linux   | `~/.local/share/Rack2/plugins-lin-x64/`             |
| macOS   | `~/Library/Application Support/Rack2/plugins-mac-*/` |
| Windows | `%LOCALAPPDATA%\Rack2\plugins-win-x64\`             |

Restart VCV Rack — Nacre's modules appear under the **Nacre** brand in the
module browser.

Alternatively, `make dist` produces a redistributable `.vcvplugin` in
`dist/` that anyone on the same OS can drop into their plugin folder
without building.

> **Want builds for all three platforms without doing it yourself?** The
> simplest route is submitting the plugin to the
> [VCV Library](https://vcvrack.com/manual/Plugin#Distributing), which
> compiles it for Linux, macOS, and Windows and lists it in Rack's in-app
> browser.

### Regenerating panels

Each module's panel SVG and `src/layout_*.hpp` are generated by its
`gen_*.py` script (labels are drawn with a built-in stroke font because
Rack's SVG renderer does not support `<text>`). Re-run the relevant script
after changing a layout, then rebuild — for example:

```bash
python3 gen_osprey.py
```
