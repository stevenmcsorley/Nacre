...
def wordmark_paths():
    """Load the already-outlined lowercase mark; Rack's SVG renderer has no text."""
    path = os.path.join(BASE, "art", "halfagiraf_wordmark.svg")
    source = open(path, encoding="utf-8").read()
    return source[source.index(">", source.index("<svg")) + 1:source.rindex("</svg>")].strip()
...
# acacia dusk artwork (traced) as the full-bleed backplate. The source is
# committed alongside this generator so regeneration does not depend on the
# original workstation or its ComfyUI directory.
art_path = os.path.join(BASE, "art", "chimera_dusk.svg")
_art = open(art_path, encoding="utf-8").read()
_vb = re.search(r'viewBox="([^"]+)"', _art).group(1).split()
